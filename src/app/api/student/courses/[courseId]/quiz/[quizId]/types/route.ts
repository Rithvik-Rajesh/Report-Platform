...
export async function GET({ params }: { params: { courseId: string, quizId: string } }) {
    try {
        ...
        // Fetch the quiz types
        const types = await db.query("SELECT * FROM quiz_types WHERE quiz_id = $1", [quizId]);
        if (types.length > 0) {
            quiz[0].types = types;
        } else {
            quiz[0].types = [];
        ...
        return NextResponse.json({ course: course[0], quiz: quiz[0], types:types });
    } catch (error) {
